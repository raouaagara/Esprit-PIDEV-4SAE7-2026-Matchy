...
export class ContractsComponent implements OnInit {
  ...
  loadContracts() {
    this.loading = true;
    this.contractService.getCompanyContracts(this.currentUser.id)
      .subscribe({
        next: (contracts) => {
  ...
  signContract(contract: Contract) {
    if (confirm('Are you sure you want to sign this contract?')) {
      this.contractService.signContract({
        ...
        signature: this.currentUser.companyName || `${this.currentUser.firstName} ${this.currentUser.lastName}`
      }).subscribe({
        next: () => {
