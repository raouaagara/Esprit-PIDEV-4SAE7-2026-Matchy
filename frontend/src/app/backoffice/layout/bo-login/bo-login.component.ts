...
export class BoLoginComponent implements AfterViewInit, OnDestroy {
  ...
  private TAGS = ['UI Design','React','Branding','Figma','Node.js','Motion','Copy','SEO','3D','Vue','Strategy'];
  ...
  onSubmit(): void {
    ...
    this.authService.login(email, password).subscribe({
      ...
      error: () => {
        this.error = 'Erreur serveur. Vérifiez que le backend tourne sur le port 8080.';
        this.isLoading = false;
      }
    });
  }
  ...
  private buildScene(): void {
    ...
    this.tags = Array.from({ length: 8 }, () => this.makeTag(true));
  }
  ...
  private drawHex(h: any): void {
    ...
    for (let k = 0; k < 6; k++) {
      ...
      const px = h.cx + h.s * Math.cos(ang);
      const py = h.cy + h.s * Math.sin(ang);
