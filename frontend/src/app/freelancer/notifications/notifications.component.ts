...
export class FlNotificationsComponent implements OnInit {
  ...
  markRead(n: Notification): void { if (!n.read && n.id) this.notifService.markAsRead(n.id).subscribe(() => n.read = true); }
  markAllRead(): void {
    const userId = this.authService.currentUser?.id;
    if (userId) this.notifService.markAllAsRead(userId).subscribe(() => this.notifications.forEach(n => n.read = true));
  }
